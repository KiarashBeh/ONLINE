...
go
-------------- steg ---------------  1 -------------------
exec msdb.dbo.TC_pGenerate_DB_User_Permissions_v4 'BI_PWC';

if exists(select 1 from sys.sysprocesses where  DB_NAME(dbid) = 'BI_PWC')
BEGIN
exec msdb..TC_pKill_DB_Sessions  @databaseName=  'BI_PWC'
END

-------------- steg ---------------  2 -------------------


exec msdb.dbo.pTC_Restore_Database_From_Prod_v4  @database_name = N'BI_PWC' 
...
GO
DECLARE @dbName nvarchar(250) = 'BI_PWC'
set nocount on;
...
while @@FETCH_STATUS = 0
	begin
	--print @sql
	SET @sql = REPLACE(@sql,'SEPWC\G_SE-RPA-Robot-DEV-DC','SEPWC\G_SE-RPA-Robot-PROD-DC');  --Byter Prod mot Dev
	--print @sql
	exec(@sql)
	fetch next from PermCur into @step, @sql
	end
