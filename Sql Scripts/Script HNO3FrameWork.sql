...
exec msdb.dbo.TC_pGenerate_DB_User_Permissions_v4 'HNO3FrameWork';

if exists(select 1 from sys.sysprocesses where  DB_NAME(dbid) = 'HNO3FrameWork')
BEGIN
exec msdb..TC_pKill_DB_Sessions  @databaseName=  'HNO3FrameWork'
END

-------------- steg ---------------  2 -------------------


exec msdb.dbo.pTC_Restore_Database_From_Prod_v4  @database_name = N'HNO3FrameWork' 
...
GO
DECLARE @dbName nvarchar(250) = 'HNO3FrameWork'
set nocount on;
...
use  [HNO3FrameWork] ;  
	...
	DECLARE GetOrphanUsers CURSOR
		...
		WHILE @@FETCH_STATUS = 0
		BEGIN
		SET @username = REPLACE(REPLACE(REPLACE(@username,'Prod','DEV'),'forecast_DEV','forecast'),'ReportUser','reportreader');  -- byter prod user till Dev user
		PRINT @username;
			IF @username='dbo'
				EXEC sp_changedbowner 'sa'
			ELSE IF @username = 'MyBusiness_DEV'
				 PRINT 'No login for MyBusiness_DEV'
			ELSE
